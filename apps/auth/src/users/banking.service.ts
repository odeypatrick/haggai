            ...
            parseString(response.data, (err, result) => {
                if (err) {
                    ...
                    const processedData = accountList.map(account => {
                        return {
                            Retval: account.Retval[0],
                            Retmsg: account.Retmsg[0],
                            AccountNo: account.AccountNo[0],
                            Accounttitle: account.Accounttitle[0],
                            CustomerID: account.CustomerID[0],
                            OldAcctNo: account.OldAcctNo[0],
                            Product: account.Product[0],
                            Branch: account.Branch[0],
                            AcctStatus: account.AcctStatus[0],
                            Address: account.Address[0],
                            PhoneNo: account.PhoneNo[0],
                            BookBalance: account.BookBalance[0],
                            AvailableBalance: account.AvailableBalance[0],
                            UnclearBal: account.UnclearBal[0],
                            PendingChg: account.PendingChg[0],
